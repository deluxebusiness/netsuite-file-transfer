...
function getData(type, objectPath, callback) {
	...
	var args = {
		...
		headers: {
			...
			"Authorization": vscode.workspace.getConfiguration('netsuiteFileTransfer')['authentication']
		}
	};

	var baseRestletURL = vscode.workspace.getConfiguration('netsuiteFileTransfer')['restlet'];

	// Support for Oath authentication
	if (!args.headers.Authorization) {
		var oauth = OAuth({
			consumer: {
				key: vscode.workspace.getConfiguration('netsuiteFileTransfer')['netsuite-key'],
				secret: vscode.workspace.getConfiguration('netsuiteFileTransfer')['netsuite-secret']
			},
		...
		var token = {
			key: vscode.workspace.getConfiguration('netsuiteFileTransfer')['consumer-token'],
			secret: vscode.workspace.getConfiguration('netsuiteFileTransfer')['consumer-secret']
		};
		var headerWithRealm = oauth.toHeader(oauth.authorize({
			url: baseRestletURL,
			method: 'POST'
		}, token));
		headerWithRealm.Authorization += ', realm="' + vscode.workspace.getConfiguration('netsuiteFileTransfer')['realm'] + '"';
